function H = MGBO_Func_V(Zinit,nbits, n, L, P, R, E, B, V_j, L_j)
% 参数说明：
% 输入：
%   nbits  - 哈希码位数
%   n      - 样本量
...
opts_BPGM.rho = 1e-2;          % 正则化系数
opts_BPGM.maxItr = 200;          % 外层最大迭代次数
opts_BPGM.record = 1;          % 是否记录过程
opts_BPGM.mxitr = 1000;         % 内层最大迭代次数
...
opts_BPGM.mu = 1e-2;%1e;              % 选择固定mu值（原代码中mu_values的默认第一个值）
...
H = OptStiefelGBB(Bk, @obj_func_AMSH_V, @moreau_hc, opts_BPGM, L, P, R, E, n, B, V_j, L_j);
